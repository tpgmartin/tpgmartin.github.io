...
In the [first post](http://tpgmartin.com/neural-networks-from-scratch-part-1-building-the-network) in this series, we created a deep learning neural network so we have everything in place to actually solve the learning problem we originally introduced: how to create a neural network to correctly match the output of an XOR gate. To do this we need adjust our network weights in response to the training data, this is what the backpropagation algorithm does. The backpropagation algorithm follows in order,
 
1. Forward propagation: Pass input into our network, and get some output
2. Error calculation: Find the error for this output using a particular error function
...
And iteratively through these four step until the network converges to the optimal set of weights for the learning problem. This post will take us through a single run of this algorithm.
...
3. Output node, O = H • w_ho = relu([A, B] • [w_ih_A, w_ih_B]) * w_ho
 
This is exactly the same process we would use to find out the value for O, our output value, for all nodes and weights.
...
From the previous step, we got some output from the network - but how do we know if this is any good? We need to be able to find out the how closely this matches the training data output values, or in other words error that our network output produces. After all, we want our to learn the training data and as we said in part 1, learning is just a process of error minimisation. To quantify the network error we are going to be using something equivalent to the mean squared error,

```math
error = (output - target) ** 2
```

Our error will only ever be a value greater than or equal to zero, where the lower the value the better our network is doing. Think about it this way, if I shoot an arrow at a target, I don’t care if I’m above/below or left/right of the target, just by how much I missed. 
 
You may still have the lingering question as to why we introduce this “extra” quantity of error, when the network weights are what we are really worried about. It is simply that the error is very easy to track and see improvements in the network. Think of it this way, if my error increases we know the network is doing worse, and conversely if it increases. If instead we just tracked the weights, we wouldn’t necessarily know what a given change in the weights meant for our network’s performance. Furthermore, as we’ll see in the next section an error function like mean squared error is guaranteed to convergence - at least for some choice of parameters. This implies that we can achieve the optimal set of weights for the learning problem with this approach.
...
for (let iteration = 0; iteration < 60; iteration++) {
    ...
    for (let i = 0; i < inputs.shape[0]; i++) {
        ...
        error = nj.add(outputLayerError, nj.sum(nj.power((nj.subtract(outputLayer, outputs.slice([i, i + 1]))), 2)
        ))
...
At this point we have some output from our network and an error. So we know how well our network is doing, but how do we improve this error? It turns out that we can only change the weights. Think about it this way, we have at our disposal the weights and the training data. We don’t want to change the training data as we want this to be an accurate reflection of the problem at hand. If we tweak the training data to get the network to train more efficiently we are only selling ourselves short: Once we move to the test case, the network would likely underperform as it not learnt data matching the learning problem. So the conclusion is that we should only think about changing the weights during this process.
...
where as before we have placeholder variables for each node value as well as for the weights, where w_ho is the weight connecting the hidden layer node to the output not, and w_ih is the node connecting the input node to the hidden layer node.
...
Indicated on the graph is the minimum of the error function, where the error is zero, which is the point we want to reach. The corresponding weight that minimises the error is the goal weight. To reach the minimum from an arbitrary point on the parabola, we need to ask ourselves, what the change in the error is for a given change in the weight value. The approach that gradient descent takes is greedy, it just takes the change in the weight that change the error most. This is just a verbose way of saying we need to find the derivative to determine the weight update. Indicated below, the derivative is the slope of the line at the point given by the dashed lines.
...
The derivative is is calculated from the expression for the error function given above, which in our case is,

```math
derivative ~ H * (O - target)
```
 
We now know how much to update the weight, which is just the derivative, but do we add or subtract this amount from the current weight value? Going back the parabola that gives the relationship between the weight and the error we find that we add or subtract this update depending what side of the goal weight we find ourselves. If we are to the left of the goal weight, the derivative is always negative, as the graph is sloping downwards so our derivative is negative. However, in order to decrease the error, we need to increase the weight to move towards the goal weight. This means we should take the negative of the derivative as our weight update. The converse argument holds if we start to the right of the goal weight: The derivative is positive, but we need to decrease the weight to decrease the error. Both scenarios are illustrated below.

{% include image.html url="/assets/media/error_function_weight_update.png" %}

Putting this altogether, the weight update for weights connecting the hidden layer nodes and output nodes is given by,

{% highlight js %}
weight -= activation * (output - target)
{% endhighlight %}
 
If we were to repeat this process for many iterations, we simply continue until we converge to this goal weight. A consequence of the error function we’ve chosen for our network, a parabola, means that the goal weight is in fact a global minimum, meaning that as long as we can guarantee convergence, which is something we’ll return to later.
...
Altogether, these derivatives give us all the weight updates we need for our network. But do these converge to a set of goal weights? Above, I’ve teased that this might be possible, but there is a possibility that convergence may not occur. This is because all the weight updates we have scalea with the activation, meaning that there is a risk of divergence if the activation grows too large (these are not normalised). For instance, the derivative the error function with respect to w_ho was found to be,
...
There are some important considerations to make when choosing alpha, as it has a direct impact on the rate of convergence of the weights. This is by no means a trivial problem in general. However, for our purposes it will be ok to just choose a fixed number between 0 and 1. Putting this altogether, our code for backpropagation and gradient descent is,
...
We've covered a lot of material in these two blog posts. Taking it from the top, we chose a learning problem, the XOR logic gate, and matched it to a deep learning neural network. We we able to justify our choices for the network such as the error function and other parameters. Having done all this we worked through a single iteration of the backpropagation algorithm to demonstrate how we would update our network weights by adjusting the weights.
